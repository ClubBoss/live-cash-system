...
| RT-05 | W11 deployed-smoke integrity | canonical `/`, `/tools` | Exact-head run #929 exposed a stale version assertion, and follow-up release audit found production/completion smoke still assumed canonical `/` was the legacy shell. A successful merge would therefore have failed exact-SHA deploy smoke or forced the smoke to validate the wrong surface. | P1 | Run #929 on `c4f98e1f206deab0eeac3346662ff325de5a5b10`: static gate GREEN; retained E2E failed only `version-closure.spec.mjs` in Chromium/mobile because the expected legacy footer was absent from the correctly rendered Practical route. Artifact review then showed `production-smoke.mjs` and `wave-d-completion-smoke.mjs` carried the same stale root contract. | Canonical-route cutover did not propagate build-identity and deployed-smoke ownership from legacy `LiveCashApp` to the new primary Practical layout. | Release evidence / deploy smoke / identity | Exact-SHA deployed release certification | NONE | NONE | Shared `BuildIdentityFooter` exposes one exact build marker on Practical Mastery and `/tools`; version gate checks both surfaces; production smoke verifies canonical Practical root + support tools; Wave-D completion smoke explicitly runs on `/tools`; static unit test locks these route contracts. | CLOSED_PENDING_EXACT_HEAD_AND_DEPLOY_PROOF |

## Current machine severity counts

- P0 found in this closure slice: 0; closed: 0; remaining: 0.
- P1 found: 3 (RT-02, RT-03, RT-05); bounded repairs implemented; remaining machine-actionable P1: 0 pending exact-head/main/deploy proof.
...
The two passes recorded against run #902 remain valid evidence for the source/evidence/runtime state they inspected, but they are not the final acceptance passes because later audits found RT-02, RT-03 and RT-05. They must not be used to claim final Gate F for a later SHA.
...
After RT-02 through RT-05, no additional bounded machine-actionable P0/P1 or positive-EV P2 is known from the frozen repair-set audit. Final closure still requires exact-head GREEN, two fresh final adversarial passes with zero new P0/P1, exact-main GREEN, exact-SHA canonical Workers deployment, deployed smoke and release-truth reconciliation. Residual BL-11 and human/empirical items cannot be manufactured without violating source/evidence integrity.
