...
export const finalContentDeltaDecisions: PracticalDecision[] = [
  ...
  },
  {
    ...
    actionOptions: [
      ...
      o("c", "Полный olл-ин уже на этой улице", "A full all-in already on this street", "DEPTH_MISREAD"),
    ],
    reasonOptions: [
      ...
      o("r3", "SPR≈4 — это уже olл-ин глубина сама по себе", "SPR≈4 is already all-in depth by itself", "DEPTH_MISREAD"),
    ],
    correctActionId: "a", correctReasonId: "r1", targetSeconds: 25,
    explanationRu: "FTGU-E01/LCM-01 geometry: примерно pot/pot соответствует SPR≈4. Больший SPR требует бóльшую долю pot на каждой улице, чтобы естественно дойти до stacks за два действия.",
  ...
  },
  {
    ...
    assumptions: ["тот же pot в двух сравнениях", "remaining stack удваивается", "двухулочный stack-off plan"],
    ...
    actionOptions: [
      o("a", "Вверх — нужна бóльшая доля pot на каждой улице", "Up — a larger pot fraction is needed on each street"),
    ...
    reasonOptions: [
      o("r1", "Больший remaining stack при том же pot повышает SPR, а более высокий SPR требует бóльшую двухулочную долю pot", "A larger remaining stack at the same pot raises SPR, and a higher SPR requires a larger two-street pot fraction"),
  ...
  },
  {
    ...
    questionRu: "Какой SPR должен определять двухулочный sizing plan на терне?",
    ...
    explanationRu: "LCM-01: nominal 200bb — не рабочая geometry. Preflop/flop action уже сжал remaining stack до SPR≈2, и именно current SPR (execution anchor ≈0.62, класс b60/b60) должен вести двухулочный plan на терне.",
  ...
  // ---------------------------------------------------------------------
  {
    ...
    correctActionId: "a", correctReasonId: "r1", targetSeconds: 28,
    explanationRu: "FTGU-E09/E10: raising range строится по urgency и board ownership, а не по одному universal ярлыку 'raise = сила'. Условная структура заменяет lозунг.",
  ...
  },
  {
    ...
    actionOptions: [
      o("a", "Состав raising range — против малого c-bet он обычно шире и мягче, против крупного — уже́ и сильнее", "The raising range's composition — against a small c-bet it is usually wider and softer, against a large one it is narrower and stronger"),
  ...
  },
  {
    ...
    questionRu: "Как board ownership влияет на то, насколько 'legitimately strong' может быть эта raising range?",
    questionEn: "How does board ownership affect how legitimately strong this raising range can be?",
    actionOptions: [
      o("a", "Low connected board поддерживает больше real strong raises, чем dry high board, где caller-friendly региона меньше", "A low connected board supports more genuinely strong raises than a dry high board, where the caller-friendly region is smaller"),
    ...
    correctActionId: "a", correctReasonId: "r1", targetSeconds: 28,
    explanationRu: "FTGU-E09/E10: board ownership — часть raise construction. Low/connected boards чаще caller-friendly и поддерживают больше legitimately strong raises, чем dry high boards.",
  ...
  },
  {
    ...
    correctActionId: "a", correctReasonId: "r1", targetSeconds: 28,
    explanationRu: "Branch-specific наблюдения материально сдвигают continuing threshold: passive underbluffer оставляет меньше real bluffs в raising range, и marginal bluff-catchers становятся менее прибыльными call.",
  ...
  },
  {
    ...
    reasonOptions: [
      o("r1", "Branch-specific профиль напрямую меняет natural bluff supply в raising range; меньше bluffs — уже continuing region для того же price", "A branch-specific profile directly changes the natural bluff supply in the raising range; fewer bluffs mean a tighter continuing region at the same price"),
  ...
  },
  {
    ...
    reasonOptions: [
      o("r1", "Card, которая completing draw или uncapping caller region, добавляет реальные strong hands и natural bluffs в raising range; brick этого не делает", "A card that completes a draw or uncaps the caller's region adds genuine strong hands and natural bluffs to the raising range; a brick does not"),
  ...
  },
  {
    ...
    cueRu: "Hero уже вложил крупный call на flop и bet на терне; теперь terн raise делает продолжение особенно дорогим при текущем SPR.",
  ...
  },
  {
    ...
    reasonOptions: [
      ...
      o("r3", "Suit texture автоматически расширяет nut region для agressor", "Suit texture automatically widens the nut region for the aggressor", "BOARD_LABEL_SHORTCUT"),
    ],
    correctActionId: "a", correctReasonId: "r1", targetSeconds: 28,
