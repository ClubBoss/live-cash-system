...
q("PM-W4-BOARD-01-101","W4-BOARD-01","recognition",["FTGU-E07"],"BTN vs BB, high dry flop that preserves raiser's high-card/overpair density.","BTN vs BB, high dry flop that preserves the raiser's high-card/overpair density.","Что распознать первым?","What should be recognized first?","Сильное сохранение преимущества диапазона","Strong preservation of range advantage","«Высокий флоп — всегда ставим»","'High flop = always bet'","Board strategy starts with how the flop interacts with both preflop ranges, not with a texture nickname."),
...
q("PM-W4-BOARD-01-103","W4-BOARD-01","decision",["FTGU-E07","SLC-SRP-BOARD-CLASSES"],"Two flops are both 'low', but one is disconnected and one highly connected.","Two flops are both 'low', but one is disconnected and one highly connected.","Можно ли дать им одну strategy только по rank height?","Can they share one strategy based only on rank height?","Нет — важны связность доски и то, как она подходит диапазонам","No — connectivity and range fit matter","Да — все низкие доски относятся к одному классу","Yes — low is one class","Board labels that ignore actual range interaction are insufficient for choosing a strategy."),
q("PM-W4-BOARD-01-104","W4-BOARD-01","decision",["FTGU-E07"],"Same flop texture, but preflop ranges change from BTN-vs-BB to tighter HJ-vs-BTN.","Same flop texture, but preflop ranges change from BTN-vs-BB to tighter HJ-vs-BTN.","Сохраняется ли board ownership автоматически?","Does board ownership remain automatic?","Нет — пересчитать через arriving ranges","No — recalculate through arriving ranges","Да — текстура доски сама определяет, кому она выгодна","Yes — board texture owns itself","A board has no strategy identity independent of the ranges that arrive there."),
...
q("PM-W4-RUNOUT-01-103","W4-RUNOUT-01","decision",["FTGU-E20"],"IP checked back flop, removing many nutted flop hands; turn is neutral.","IP checked back flop, removing many nutted flop hands; turn is neutral.","Что проверить до probe?","What should be checked before probing?","Какие сильные руки были удалены из диапазона чек-бэком на флопе","Which strong hands the flop check removed","Только то, что c-bet был пропущен","Only that the c-bet was missed","Probe opportunity comes from the exact check-back composition, not from a mechanical missed-c-bet rule."),
q("PM-W4-RUNOUT-01-104","W4-RUNOUT-01","decision",["FTGU-E20"],"Flop check-back looked capped, but turn strongly improves the check-back range.","The flop check-back looked capped, but the turn strongly improves the check-back range.","Нужно ли mechanical overbet probe?","Should Hero mechanically overbet probe?","Нет — нужно заново оценить, остался ли диапазон капнутым","No — the cap must be reassessed","Да — пропущенный c-bet всегда означает овербет","Yes — a missed c-bet always means overbet","The turn card can strengthen and uncap the checked-back range."),
...
q("PM-W4-HAND-01-106","W4-HAND-01","changed",["FINAL_LEARNING_INTEGRITY"],"Exact rank pattern stays similar, but board changes vulnerability/redraws.","The exact rank pattern stays similar, but the board changes vulnerability/redraws.","Что обновить?","What should be updated?","Свойства семейства, а не только название","Family traits, not just the name","Ничего","Nothing","A family is useful only if its contextual traits are recomputed when the board changes.",["board","vulnerability"]),
