import { MODULE_IDS, type MeasurementContext, type ModuleId } from "./model";
...
export function parseDiagnosticScore(value: unknown): DiagnosticScoreOutput {
  ...
  for (const [lcm, unknownRow] of Object.entries(value.module_summary)) {
    ...
    if (!finite(unknownRow.exposures) || unknownRow.exposures < 1 || !Array.isArray(unknownRow.items)) throw new Error(`Invalid exposure data: ${lcm}`);
...
export function deriveDiagnosticPriorityModules(score: DiagnosticScoreOutput): ModuleId[] {
  return Object.entries(score.module_summary)
    .map(([lcm, row]) => ({ moduleId: LCM_TO_MODULE[lcm], weight: row.observed_error_rate * Math.max(1, row.exposures) }))
    .filter((row) => MODULE_IDS.includes(row.moduleId) && row.weight > 0)
