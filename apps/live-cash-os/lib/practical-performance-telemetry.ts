...
export function createPracticalPerformanceEvent(input:{decisionId:string;actionId:string;reasonId:string;confidence:number;confidenceProvenance?:PracticalConfidenceProvenance;startedAt:Date;answeredAt?:Date;mode:PracticalStimulusMode;scaffold?:PracticalPerformanceEvent["scaffold"]}):PracticalPerformanceEvent{
 ...
 return {id:`${input.decisionId}:${answeredAt.toISOString()}`,decisionId:input.decisionId,skillId:decision.skillId,mode:input.mode,startedAt:input.startedAt.toISOString(),answeredAt:answeredAt.toISOString(),responseMs:Math.max(0,answeredAt.getTime()-input.startedAt.getTime()),confidence:Math.max(0,Math.min(100,Math.round(input.confidence))),...(input.confidenceProvenance?{confidenceProvenance:input.confidenceProvenance}:{}),actionCorrect,reasonCorrect,correct:actionCorrect&&reasonCorrect,kind:decision.kind,scaffold:input.scaffold};
}
