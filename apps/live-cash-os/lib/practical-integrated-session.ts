...
import {
  learningRouteScore,
  whyNowForSkill,
} from "../content/practical-mastery/learning-route";
import {
...
export type IntegratedSessionItem = {
  decisionId: string;
  skillId: string;
  priority: number;
...
function selectedMisconceptions(attempt: PracticalAttempt): string[] {
  ...
  if (!decision) return [];
  const action = decision.actionOptions.find((option) => option.id === attempt.actionId);
  const reason = decision.reasonOptions.find((option) => option.id === attempt.reasonId);
  return [action?.misconception, reason?.misconception].filter((value): value is string => Boolean(value));
}

function latestAttemptsByDecision(state: PracticalMasteryState): Map<string, PracticalAttempt> {
...
export function unresolvedMistakeFamilies(state: PracticalMasteryState): MistakeFamily[] {
  ...
  for (const attempt of latest.values()) {
    ...
    const keys = selectedMisconceptions(attempt);
    const familyKeys = keys.length ? keys : [`SKILL:${attempt.skillId}`];
    for (const key of familyKeys) {
      const composite = `${attempt.skillId}:${key}`;
      ...
      current.priority += attempt.confidence >= HIGH_CONFIDENCE_WRONG ? 5 : 2;
...
function latestCorrectAttempt(state: PracticalMasteryState, skillId: string): PracticalAttempt | null {
  return [...state.attempts].reverse().find((attempt) => attempt.skillId === skillId && attempt.correct) ?? null;
}
...
export function retentionTierDue(state: PracticalMasteryState, skillId: string, now = new Date()): number | null {
  ...
  if (!progress || !stageAtLeast(progress.evidenceStage, "BOUNDARY_TESTED")) return null;
...
function candidateDecisionForSkill(
  ...
  const fresh = pool.find((decision) => !attempted.has(decision.id) && (!requireNonIdenticalToLatest || decision.id !== latest?.decisionId));
  if (fresh) return fresh;
  return pool.find((decision) => !requireNonIdenticalToLatest || decision.id !== latest?.decisionId) ?? null;
}

function currentStage(state: PracticalMasteryState, skillId: string): PracticalEvidenceStage {
  return state.skills[skillId]?.evidenceStage ?? "SOURCE_SUPPORTED";
}

function recentExposurePenalty(state: PracticalMasteryState, skillId: string): number {
  const count = state.attempts.slice(-12).filter((attempt) => attempt.skillId === skillId).length;
...
export function buildIntegratedSession(state: PracticalMasteryState, now = new Date(), size = INTEGRATED_SESSION_SIZE): IntegratedSessionItem[] {
  ...
  const push = (decision: PracticalDecision, reason: IntegratedSessionItem["reason"], priority: number, why: string, retentionTierDays: number | null = null) => {
    ...
    excluded.add(decision.id);
    skillUse.set(decision.skillId, (skillUse.get(decision.skillId) ?? 0) + 1);
  };
  ...
  const rankedSkills = [...eligibleIds].map((skillId) => {
    ...
    return {
      skillId,
      stage,
      score: learningRouteScore({ skill, currentStage: stage }) - recentExposurePenalty(state, skillId),
      why: whyNowForSkill(skill, stage),
    };
  ...
  for (const item of rankedSkills) {
    if (items.length >= size) break;
    const transferReady = stageAtLeast(item.stage, "DECISION_TRAINED");
    ...
    const decision = candidateDecisionForSkill(state, item.skillId, kinds, excluded, false);
...
export function recordIntegratedDecision(
  state: PracticalMasteryState,
  item: IntegratedSessionItem,
  input: { actionId: string; reasonId: string; confidence: number; now?: Date },
): PracticalMasteryState {
  const now = input.now ?? new Date();
  const decision = practicalDecisionById.get(item.decisionId);
  if (!decision) throw new Error(`Unknown integrated decision: ${item.decisionId}`);
  const latestCorrectBefore = latestCorrectAttempt(state, decision.skillId);
...
export type RealHandRepairSignals = {
  ...
  potType?: "srp" | "3bp" | "4bp" | "multiway";
  role?: "aggressor_ip" | "aggressor_oop" | "caller_ip" | "caller_oop";
  blindIssue?: boolean;
  openSizeIssue?: boolean;
  boardOwnershipIssue?: boolean;
  automaticCbetIssue?: boolean;
  probeIssue?: boolean;
  bluffCatchIssue?: boolean;
  multiwayThresholdIssue?: boolean;
  straddleGeometryIssue?: boolean;
  evidenceGeneralizationIssue?: boolean;
};
...
export function routeRealHandToRepairs(signals: RealHandRepairSignals): RealHandRepairCandidate[] {
  const candidates: RealHandRepairCandidate[] = [];
  const add = (skillId: string, reason: string, priority: number) => { if (!candidates.some((item) => item.skillId === skillId)) candidates.push({ skillId, reason, priority }); };
  ...
  if (signals.potType === "3bp") {
    const byRole: Record<NonNullable<RealHandRepairSignals["role"]>, string> = { aggressor_ip: "3BP-01", aggressor_oop: "3BP-02", caller_ip: "3BP-03", caller_oop: "3BP-04" };
    if (signals.role) add(byRole[signals.role], "3-bet-pot role-specific execution", 86);
    add("3BP-05", "3-bet-pot board/sizing matrix", 80);
  }
