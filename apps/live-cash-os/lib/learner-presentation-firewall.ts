...
const leakPatterns: ReadonlyArray<{
  ...
  },
  {
    ...
    pattern: /\b(?:POSITIVE_EV_SOURCE_ACCESS_REQUIRED|sourceRefs|source[- ]backed|source[- ]supported|source\s+integrity|source\s+mechanism|source\s+scope|field[- ]gated|inspectable\s+source|underlying\s+material|source\s+material)\b|(?:источник\s+подтверждает|исходн\w*\s+материал\w*|доступн\w*\s+материал\w*|(?:solver|course)-источник\w*)/iu,
  },
  {
    leakClass: "VALIDATION_PIPELINE",
    pattern: /\b(?:structured\s+canonical\s+binding|structured\s+causal\s+classification|visual\s+claim\s+review|targeted\s+visual\s+extraction|routing\s+inventory)\b/iu,
  },
  {
...
function replacePhrases(value: string, locale: LearnerPresentationLocale): string {
  const replacements: Array<[RegExp, string]> = locale === "ru"
    ? [
        ...
        [/\bExact\s+frequencies\s+not\s+yet\s+verified\b/gu, "Точные частоты здесь пока не установлены"],
    ...
    : [
        ...
        [/\bExact\s+frequencies\s+not\s+yet\s+verified\b/gu, "Exact frequencies are not established here yet"],
