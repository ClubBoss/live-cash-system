...
import { isCurrentPracticalEvidenceAttempt, latestAttemptsByDecision, practicalAttemptedDecisionIds, type PracticalMasteryState } from "./practical-mastery-core";
...
function buildGenericAdaptiveSession(state:PracticalMasteryState,now:Date,size:number,performance:PracticalPerformanceSample[]):IntegratedSessionItem[]{
  ...
  for(const need of needs){
    ...
    const latest=latestPhysical&&isCurrentPracticalEvidenceAttempt(latestPhysical)?latestPhysical:null;
...
function buildFocusedIntegratedSession(state:PracticalMasteryState,now:Date,size:number,performance:PracticalPerformanceSample[],skillId:string):IntegratedSessionItem[]{
  ...
  const attemptedFamilies=new Set([...practicalAttemptedDecisionIds(state)].flatMap((decisionId)=>{
    const decision=practicalDecisionById.get(decisionId);
    return decision&&isOrdinaryLearnerDecision(decision)&&decision.skillId===skillId?[practicalStimulusFamilyId(decision)]:[];
  }));
