import { practicalAnchors, practicalDecisionById, practicalDecisions, practicalSkillById, practicalSkillFamilies } from "../content/practical-mastery";
...
function distinctSuccessfulByKind(progress: PracticalSkillProgress, kinds: PracticalDecision["kind"][]): number {
  return new Set(progress.successfulDecisionIds.filter((decisionId) => { const decision = practicalDecisionById.get(decisionId); return decision ? kinds.includes(decision.kind) : false; })).size;
}
...
export function createPracticalMasteryState(now = new Date(), resetFromLegacy = false): PracticalMasteryState {
  const skills = Object.fromEntries(practicalSkillFamilies.map((skill) => [skill.id, {
    skillId: skill.id, evidenceStage: "SOURCE_SUPPORTED" as const, conceptTaught: false, conceptTaughtAt: null,
    recognitionCorrect: 0, directDecisionCorrect: 0, changedCorrect: 0, boundaryCorrect: 0, mixedCorrect: 0,
    successfulDecisionIds: [], retentionDaysPassed: [], delayedRetrievalPassed: false, realHandTransferReviewed: false,
    attempts: 0, correct: 0, lastAttemptAt: null, lastIncorrectDecisionId: null,
  }]));
...
export function practicalPrerequisitesMet(state: PracticalMasteryState, skillId: string): boolean {
  ...
  return hardDependenciesFor(skillId).every((dependency) => { const progress = state.skills[dependency.fromSkillId]; return progress ? stageAtLeast(progress.evidenceStage, "DECISION_TRAINED") : false; });
}
...
export function trainablePracticalSkills(state: PracticalMasteryState) { return availablePracticalSkills(state).filter((skill) => practicalSkillCorpusCanReach(skill.id, "DECISION_TRAINED")); }

function repairUrgencyForSkill(state: PracticalMasteryState, skillId: string): 0 | 1 | 2 | 3 {
  const latest = latestAttemptsByDecision(state, skillId); const wrong = [...latest.values()].filter((attempt) => !attempt.correct).length; if (wrong >= 4) return 3; if (wrong >= 2) return 2; if (wrong >= 1) return 1; return 0;
}
...
function softReadinessPenalty(state: PracticalMasteryState, skillId: string): number {
  return softDependenciesFor(skillId).reduce((penalty, dependency) => {
    const progress = state.skills[dependency.fromSkillId];
    if (!progress?.conceptTaught) return penalty + 7;
    if (!stageAtLeast(progress.evidenceStage, "RECOGNITION_TRAINED")) return penalty + 4;
    return penalty;
  }, 0);
}
...
export function recommendNextPracticalSkill(state: PracticalMasteryState): PracticalRecommendedSkill | null {
  ...
  const urgentRepair = trainable.map((skill) => ({ skill, urgency: repairUrgencyForSkill(state, skill.id) })).filter((candidate) => candidate.urgency >= 2).sort((a, b) => b.urgency - a.urgency)[0];
  if (urgentRepair) {
    const progress = state.skills[urgentRepair.skill.id];
    return { skillId: urgentRepair.skill.id, score: learningRouteScore({ skill: urgentRepair.skill, currentStage: progress.evidenceStage, repairUrgency: urgentRepair.urgency }), whyNow: whyNowForSkill(urgentRepair.skill, progress.evidenceStage, urgentRepair.urgency) };
  }

  for (const skillId of canonicalFirstJourneySkillIds) {
    const skill = trainable.find((candidate) => candidate.id === skillId); const progress = skill ? state.skills[skill.id] : null;
    if (skill && progress && !stageAtLeast(progress.evidenceStage, "RECOGNITION_TRAINED")) return { skillId: skill.id, score: learningRouteScore({ skill, currentStage: progress.evidenceStage }), whyNow: whyNowForSkill(skill, progress.evidenceStage) };
  }

  const newCapability = trainable
    .filter((skill) => !stageAtLeast(state.skills[skill.id]?.evidenceStage ?? "SOURCE_SUPPORTED", "RECOGNITION_TRAINED") && hasTeachingAnchor(skill.id))
    .map((skill) => { const progress = state.skills[skill.id]; return { skillId: skill.id, score: learningRouteScore({ skill, currentStage: progress.evidenceStage }) - softReadinessPenalty(state, skill.id), whyNow: whyNowForSkill(skill, progress.evidenceStage) }; })
    .sort((a, b) => b.score - a.score || a.skillId.localeCompare(b.skillId))[0];
  if (newCapability) return newCapability;

  const ranked = trainable.map((skill) => {
    const progress = state.skills[skill.id]; const repairUrgency = repairUrgencyForSkill(state, skill.id); const recentExposurePenalty = recentExposurePenaltyForSkill(state, skill.id);
    return { skillId: skill.id, score: learningRouteScore({ skill, currentStage: progress.evidenceStage, repairUrgency, recentExposurePenalty }) - softReadinessPenalty(state, skill.id), whyNow: whyNowForSkill(skill, progress.evidenceStage, repairUrgency) };
...
export function recordPracticalDecision(state: PracticalMasteryState, input: { decisionId: string; actionId: string; reasonId: string; confidence: number; now?: Date }): PracticalMasteryState {
  ...
  if (correct) {
    nextProgress.correct += 1; if (!nextProgress.successfulDecisionIds.includes(decision.id)) nextProgress.successfulDecisionIds.push(decision.id);
    if (decision.kind === "recognition") nextProgress.recognitionCorrect += 1; if (decision.kind === "decision") nextProgress.directDecisionCorrect += 1; if (decision.kind === "changed") nextProgress.changedCorrect += 1; if (decision.kind === "boundary") nextProgress.boundaryCorrect += 1; if (decision.kind === "mixed") nextProgress.mixedCorrect += 1;
    if (nextProgress.lastIncorrectDecisionId === decision.id) nextProgress.lastIncorrectDecisionId = null;
  } else nextProgress.lastIncorrectDecisionId = decision.id;
...
export function nextPracticalDecision(state: PracticalMasteryState, skillId: string): PracticalDecision | null {
  if (!practicalPrerequisitesMet(state, skillId)) return null; const pool = decisionsForPracticalSkill(skillId); if (!pool.length) return null; const progress = state.skills[skillId];
...
export function practicalRepairQueue(state: PracticalMasteryState): string[] {
  const latest = latestAttemptsByDecision(state); const bySkill = new Map<string, PracticalAttempt[]>(); for (const attempt of latest.values()) { if (attempt.correct) continue; const attempts = bySkill.get(attempt.skillId) ?? []; attempts.push(attempt); bySkill.set(attempt.skillId, attempts); }
  return [...bySkill.entries()].sort((left, right) => right[1].length - left[1].length || left[0].localeCompare(right[0])).map(([skillId]) => skillId);
}
