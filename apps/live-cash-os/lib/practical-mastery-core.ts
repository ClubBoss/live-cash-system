...
export function trainablePracticalSkills(state: PracticalMasteryState) { return availablePracticalSkills(state).filter((skill) => !isPracticalBridgeSkill(skill.id) && practicalSkillCorpusCanReach(skill.id, "DECISION_TRAINED")); }
...
export function recommendNextPracticalSkill(state: PracticalMasteryState): PracticalRecommendedSkill | null {
  ...
  if (urgentRepair) { const progress = state.skills[urgentRepair.skill.id]; return { skillId: urgentRepair.skill.id, score: learningRouteScore({ skill: urgentRepair.skill, currentStage: progress.evidenceStage, repairUrgency: urgentRepair.urgency }), whyNow: whyNowForSkill(urgentRepair.skill, progress.evidenceStage, urgentRepair.urgency) }; }
...
export function practicalRepairQueue(state: PracticalMasteryState): string[] { const latest = latestAttemptsByDecision(state); const bySkill = new Map<string, PracticalAttempt[]>(); for (const attempt of latest.values()) { if (attempt.correct || isPracticalBridgeSkill(attempt.skillId)) continue; const attempts = bySkill.get(attempt.skillId) ?? []; attempts.push(attempt); bySkill.set(attempt.skillId, attempts); } return [...bySkill.entries()].sort((left, right) => right[1].length - left[1].length || left[0].localeCompare(right[0])).map(([skillId]) => skillId); }
