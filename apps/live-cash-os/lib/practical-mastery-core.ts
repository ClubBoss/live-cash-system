...
export function nextPracticalDecision(state: PracticalMasteryState, skillId: string): PracticalDecision | null {
  ...
  if (distinctSuccessfulByKind(progress, ["recognition"]) < MIN_RECOGNITION_STIMULI || distinctSuccessfulScenariosByKind(progress, ["recognition"]) < MIN_RECOGNITION_SCENARIOS) return unattemptedDecisionOfKinds(state, skillId, ["recognition"], new Set<string>(), true); if (distinctSuccessfulByKind(progress, ["decision"]) < MIN_DIRECT_DECISION_STIMULI || distinctSuccessfulScenariosByKind(progress, ["decision"]) < MIN_DIRECT_DECISION_SCENARIOS) return unattemptedDecisionOfKinds(state, skillId, ["decision"], new Set<string>(), true); if (distinctSuccessfulByKind(progress, ["changed", "mixed"]) < MIN_TRANSFER_STIMULI || distinctSuccessfulScenariosByKind(progress, ["changed", "mixed"]) < MIN_TRANSFER_SCENARIOS) return unattemptedDecisionOfKinds(state, skillId, ["changed", "mixed"], new Set<string>(), true); if (distinctSuccessfulByKind(progress, ["boundary"]) < MIN_BOUNDARY_STIMULI) return unattemptedDecisionOfKinds(state, skillId, ["boundary"]);
