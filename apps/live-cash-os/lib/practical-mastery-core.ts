...
function distinctSuccessfulByKind(progress: PracticalSkillProgress, kinds: PracticalDecision["kind"][]): number { return new Set(progress.successfulDecisionIds.flatMap((decisionId) => { const decision = practicalDecisionById.get(decisionId); return decision && decision.skillId === progress.skillId && kinds.includes(decision.kind) ? [practicalEvidenceFamilyId(decision)] : []; })).size; }
...
export function deriveEvidenceStage(progress: PracticalSkillProgress): PracticalEvidenceStage {
  ...
  const recognitionScenarios = distinctSuccessfulScenariosByKind(progress, ["recognition"]); const directScenarios = distinctSuccessfulScenariosByKind(progress, ["decision"]); const transferScenarios = distinctSuccessfulScenariosByKind(progress, ["changed", "mixed"]);
  let stage: PracticalEvidenceStage;
  if (recognition < MIN_RECOGNITION_STIMULI || recognitionScenarios < MIN_SCENARIO_FAMILIES) stage = "CONCEPT_TAUGHT"; else if (direct < MIN_DIRECT_DECISION_STIMULI || directScenarios < MIN_SCENARIO_FAMILIES) stage = "RECOGNITION_TRAINED"; else if (transfer < MIN_TRANSFER_STIMULI || transferScenarios < MIN_SCENARIO_FAMILIES) stage = "DECISION_TRAINED"; else if (boundary < MIN_BOUNDARY_STIMULI) stage = "CHANGED_NODE_TRANSFER"; else if (!progress.delayedRetrievalPassed) stage = "BOUNDARY_TESTED"; else if (!progress.realHandTransferReviewed) stage = "DELAYED_RETRIEVAL"; else stage = "REAL_HAND_TRANSFER";
...
export function practicalSkillCorpusCanReach(skillId: string, stage: PracticalEvidenceStage): boolean {
  ...
  const stats = practicalSkillCorpusStats(skillId); if (stageAtLeast(stage, "RECOGNITION_TRAINED") && (stats.recognition < MIN_RECOGNITION_STIMULI || stats.recognitionScenarios < MIN_SCENARIO_FAMILIES)) return false; if (stageAtLeast(stage, "DECISION_TRAINED") && (stats.direct < MIN_DIRECT_DECISION_STIMULI || stats.directScenarios < MIN_SCENARIO_FAMILIES)) return false; if (stageAtLeast(stage, "CHANGED_NODE_TRANSFER") && (stats.transfer < MIN_TRANSFER_STIMULI || stats.transferScenarios < MIN_SCENARIO_FAMILIES)) return false; if (stageAtLeast(stage, "BOUNDARY_TESTED") && stats.boundary < MIN_BOUNDARY_STIMULI) return false; return true;
}
...
export function nextPracticalDecision(state: PracticalMasteryState, skillId: string): PracticalDecision | null {
  ...
  const recognitionScenarios = distinctSuccessfulScenariosByKind(progress, ["recognition"]); const directScenarios = distinctSuccessfulScenariosByKind(progress, ["decision"]); const transferScenarios = distinctSuccessfulScenariosByKind(progress, ["changed", "mixed"]);
  if (distinctSuccessfulByKind(progress, ["recognition"]) < MIN_RECOGNITION_STIMULI || recognitionScenarios < MIN_SCENARIO_FAMILIES) return unattemptedDecisionOfKinds(state, skillId, ["recognition"], new Set<string>(), recognitionScenarios < MIN_SCENARIO_FAMILIES); if (distinctSuccessfulByKind(progress, ["decision"]) < MIN_DIRECT_DECISION_STIMULI || directScenarios < MIN_SCENARIO_FAMILIES) return unattemptedDecisionOfKinds(state, skillId, ["decision"], new Set<string>(), directScenarios < MIN_SCENARIO_FAMILIES); if (distinctSuccessfulByKind(progress, ["changed", "mixed"]) < MIN_TRANSFER_STIMULI || transferScenarios < MIN_SCENARIO_FAMILIES) return unattemptedDecisionOfKinds(state, skillId, ["changed", "mixed"], new Set<string>(), transferScenarios < MIN_SCENARIO_FAMILIES); if (distinctSuccessfulByKind(progress, ["boundary"]) < MIN_BOUNDARY_STIMULI) return unattemptedDecisionOfKinds(state, skillId, ["boundary"]);
