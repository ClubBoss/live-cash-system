...
export default function PracticalPerceptualExperience() {
  ...
  if (!table || !decision || !skill) return <main style={{ maxWidth: 760, margin: "0 auto", padding: "32px 20px 64px" }}><p className="eyebrow">{locale === "ru" ? "ЧТЕНИЕ СТОЛА" : "PERCEPTUAL PRACTICE"}</p><h1>{locale === "ru" ? "Сначала познакомься с механизмами" : "Learn the mechanisms first"}</h1><p>{locale === "ru" ? "Этот режим не проверяет незнакомые темы. Сначала пройди Быстрый старт, а затем изученные навыки появятся здесь уже как реальные состояния стола." : "This mode does not test unseen topics. Complete Quick Start first; learned skills will then appear here as table states."}</p><Link className="primary" href="/mastery/journey">{locale === "ru" ? "Быстрый старт" : "Quick Start"} →</Link></main>;
  ...
  return <main style={{ maxWidth: 900, margin: "0 auto", padding: "24px 18px 64px" }}>
    <section className="hero compact-hero"><p className="eyebrow">{requestedFocus ? (locale === "ru" ? `ТОЧНЫЙ ФОКУС · ${scaffoldLabel}` : `EXACT FOCUS · ${scaffoldLabel}`) : (locale === "ru" ? `ЧТЕНИЕ СТОЛА · ${scaffoldLabel}` : `PERCEPTUAL PRACTICE · ${scaffoldLabel}`)}</p><h1>{requestedFocus && requestedSkill ? (locale === "ru" ? requestedSkill.titleRu : requestedSkill.titleEn) : (locale === "ru" ? "Прочитай стол до того, как назовёшь тему" : "Read the table before naming the topic")}</h1>{supportCue ? <p>{supportCue}</p> : <p>{locale === "ru" ? "Подсказка снята: сам найди переменную, которая меняет решение." : "The cue has faded: identify the decision-changing variable yourself."}</p>}</section>
