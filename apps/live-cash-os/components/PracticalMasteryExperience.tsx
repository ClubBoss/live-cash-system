...
function loadState(): PracticalMasteryState {
  ...
  try {
    ...
    ) throw new Error("invalid practical state");
    return parsed;
  } catch {
...
function DecisionCard({ locale, decision, state, onState }: { locale: Locale; decision: PracticalDecision; state: PracticalMasteryState; onState: (state: PracticalMasteryState) => void }) {
  ...
  useEffect(() => {
    setActionId(""); setReasonId(""); setConfidence(65); setRevealed(false); setWasCorrect(null);
  }, [decision.id]);
...
export default function PracticalMasteryExperience() {
  ...
  return <main style={{ maxWidth: 1180, margin: "0 auto", padding: "28px 20px 60px" }}>
    <section className="hero compact-hero">
      ...
      <p className="lede">{locale === "ru" ? "Skill-graph программа Live Cash OS. Теория ведёт к распознаванию, решениям, переносу и delayed retrieval — не к галочке за просмотр." : "The Live Cash OS skill graph. Theory leads to recognition, decisions, transfer and delayed retrieval — not a completion checkbox."}</p>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><div className="mode-switch"><button aria-pressed={locale === "ru"} onClick={() => setLocale("ru")}>RU</button><button aria-pressed={locale === "en"} onClick={() => setLocale("en")}>EN</button></div><a className="secondary" href="/mastery/session">{locale === "ru" ? "Mixed session" : "Mixed session"} →</a></div>
    </section>

    <section className="metrics"><div><b>{practicalSkillFamilies.length}</b><span>skill families</span></div><div><b>{practicalDecisions.length}</b><span>{locale === "ru" ? "scored decisions сейчас" : "scored decisions now"}</span></div><div><b>{trained}</b><span>decision-trained skills</span></div><div><b>{attempted ? Math.round((correct / attempted) * 100) : 0}%</b><span>{locale === "ru" ? "точность попыток" : "attempt accuracy"}</span></div></section>

    {recommendedSkill ? <section className="today-card" style={{ marginTop: 22 }}><p className="eyebrow">{locale === "ru" ? "РЕКОМЕНДОВАНО СЕЙЧАС" : "RECOMMENDED NOW"}</p><h2>{skillTitle(recommendedSkill, locale)}</h2><p>{locale === "ru" ? recommendedSkill.objectiveRu : recommendedSkill.titleEn}</p>{recommendation?.whyNow ? <p className="support">{recommendation.whyNow}</p> : null}<button className="primary" onClick={() => setSelectedSkillId(recommendedSkill.id)}>{locale === "ru" ? "Продолжить обучение" : "Continue learning"} <span>→</span></button></section> : <section className="today-card" style={{ marginTop: 22 }}><p className="eyebrow">CONTENT READINESS</p><p>{locale === "ru" ? "Сейчас нет следующего честно trainable узла. Это content/source readiness, а не learner failure." : "There is currently no next honestly trainable node. This is content/source readiness, not learner failure."}</p></section>}

    <section className="surface" style={{ marginTop: 22 }}><div className="section-head"><p className="eyebrow">SKILL GRAPH</p><h2>{locale === "ru" ? "Карта навыков" : "Skill map"}</h2></div>{grouped.map(([wave, skills]) => <details key={wave} open={wave === skill.wave} style={{ marginBottom: 12 }}><summary style={{ cursor: "pointer", fontWeight: 700 }}>{waveLabel(wave)} · {skills.length}</summary><div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>{skills.map((item) => { const itemGap = practicalSourceGapBySkillId.get(item.id); const itemProgress = state.skills[item.id]; const locked = !availableIds.has(item.id); const trainable = trainableIds.has(item.id); return <button key={item.id} className={item.id === skill.id ? "primary" : "secondary"} onClick={() => setSelectedSkillId(item.id)} style={{ opacity: locked ? 0.58 : 1 }}>{item.id} · {skillTitle(item, locale)}{itemGap?.status === "SOURCE_BLOCKED" ? " · SOURCE BLOCKED" : repairIds.has(item.id) ? " · REPAIR" : !trainable && !locked ? " · CONTENT BUILD" : itemProgress ? ` · ${evidenceLabel(locale, itemProgress.evidenceStage)}` : ""}</button>; })}</div></details>)}</section>

    <section className="surface" style={{ marginTop: 22 }}>
      <p className="eyebrow">{skill.wave} · {skill.livePriority}</p><h1>{skillTitle(skill, locale)}</h1><p>{skill.objectiveRu}</p>
      <p className="support">{locale === "ru" ? "Целевой уровень навыка" : "Target evidence"}: {evidenceLabel(locale, skill.targetEvidenceStage)} · {locale === "ru" ? "Текущий" : "Current"}: {evidenceLabel(locale, progress?.evidenceStage ?? "SOURCE_SUPPORTED")}</p>
      <p className="support">Corpus: R {corpusStats.recognition} · D {corpusStats.direct} · T {corpusStats.transfer} · B {corpusStats.boundary}</p><p className="support">{locale === "ru" ? "Source routing" : "Source routing"}: {skill.sourceRefs.join(", ")}</p>
      ...
      {available && !decisionTrainable && (!gap || gap.status !== "SOURCE_BLOCKED") && <div className="today-card" style={{ marginTop: 14 }}><p className="eyebrow">CONTENT BUILD</p><p>{locale === "ru" ? "Узел определён и source-routed, но corpus ещё недостаточен для честного DECISION_TRAINED. Scheduler не должен выдавать его как полноценную mastery-сессию." : "The node is source-routed but its corpus cannot yet honestly prove DECISION_TRAINED. The scheduler must not present it as a full mastery session."}</p></div>}
      {!available && <p className="support">{locale === "ru" ? `Locked: сначала decision-trained HARD prerequisites — ${hardPrerequisiteIds.join(", ") || "—"}.` : `Locked: first decision-train HARD prerequisites — ${hardPrerequisiteIds.join(", ") || "—"}.`}</p>}
      {available && (!gap || gap.status !== "SOURCE_BLOCKED") && !progress?.conceptTaught && <div className="today-card" style={{ marginTop: 18 }}><p className="eyebrow">{locale === "ru" ? "СНАЧАЛА ПОЙМИ МЕХАНИЗМ" : "UNDERSTAND THE MECHANISM FIRST"}</p>{lessonAnchors.length ? lessonAnchors.map((anchor) => <div key={anchor.id} style={{ marginTop: 18 }}><h3>{locale === "ru" ? anchor.promptRu : anchor.promptEn}</h3><p><b>{locale === "ru" ? anchor.answerRu : anchor.answerEn}</b></p><p>{locale === "ru" ? anchor.rationaleRu : anchor.rationaleEn}</p><p className="support">{anchor.sourceRefs.join(", ")}</p></div>) : <p>{locale === "ru" ? "Для этого skill пока нет достаточно качественного teaching anchor. Он остаётся в graph, но не считается изученным." : "This skill does not yet have a sufficiently strong teaching anchor. It remains in the graph but is not considered taught."}</p>}{lessonAnchors.length > 0 && <button className="primary" onClick={() => setState(markPracticalConceptTaught(state, skill.id))}>{locale === "ru" ? "Понял — перейти к решениям" : "Understood — move to decisions"} <span>→</span></button>}</div>}
      {available && progress?.conceptTaught && decision && <DecisionCard locale={locale} decision={decision} state={state} onState={setState} />}
      {available && progress?.conceptTaught && !decision && (!gap || gap.status !== "SOURCE_BLOCKED") && <p className="support">{locale === "ru" ? "На текущем уровне нет следующей независимой задачи. Уровень навыка не повышается автоматически: нужны дополнительные задания, проверка после паузы или разбор реальной руки." : "There is no next independent stimulus at the current evidence stage. The skill does not advance automatically: additional content or delayed/real-hand evidence is required."}</p>}
    </section>

    <section className="integrity" style={{ marginTop: 22 }}><h2>Evidence integrity</h2><p>{locale === "ru" ? "Одна правильная задача не создаёт mastery. Нужны разные recognition-, direct-, transfer- и boundary-stimuli; delayed retention и real-hand transfer фиксируются отдельно. Legacy completion не переносится автоматически." : "One correct answer does not create mastery. Distinct recognition, direct, transfer and boundary stimuli are required; delayed retention and real-hand transfer are recorded separately. Legacy completion does not migrate automatically."}</p></section>
  </main>;
}
