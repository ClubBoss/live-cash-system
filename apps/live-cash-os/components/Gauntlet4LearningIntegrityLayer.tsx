...
function feedbackKey(feedback: FeedbackSnapshot | null): string {
  return feedback
    ? [feedback.drillId, feedback.actionOk, feedback.reasonOk, feedback.selectedActionOptionId, feedback.selectedReasonOptionId, feedback.confidence].join(":")
    : "";
}
...
function sameSnapshot(left: IntegritySnapshot, right: IntegritySnapshot): boolean {
  return left.locale === right.locale
    ...
    && feedbackKey(left.feedback) === feedbackKey(right.feedback);
}
...
function readSnapshot(): IntegritySnapshot {
  ...
  try {
    ...
    const feedback = interaction
      && typeof interaction.drillId === "string"
      && typeof interaction.actionOk === "boolean"
      && typeof interaction.reasonOk === "boolean"
      && typeof interaction.selectedActionOptionId === "string"
      && typeof interaction.selectedReasonOptionId === "string"
      ? {
          drillId: interaction.drillId,
          actionOk: interaction.actionOk,
          reasonOk: interaction.reasonOk,
          selectedActionOptionId: interaction.selectedActionOptionId,
          selectedReasonOptionId: interaction.selectedReasonOptionId,
          confidence: typeof interaction.confidence === "number" ? interaction.confidence : 65,
        }
      : null;
    return {
      locale,
      ...
      step: typeof session?.step === "number" ? session.step : null,
...
function deterministicShuffle<T>(items: T[], seedValue: string): T[] {
  ...
  if (result.length > 1 && result.every((item, index) => item === items[index])) {
    result.unshift(result.pop() as T);
  }
  return result;
}
...
function OrderingExercise({ host, locale, moduleId, startedAt }: {
  host: HTMLElement;
  locale: LocaleCode;
  moduleId: ModuleId;
  startedAt: string;
  ...
  const copy = locale === "ru" ? {
    eyebrow: "4 · ПОРЯДОК РЕШЕНИЯ",
    title: "Собери шаги в рабочем порядке",
    help: "Перемещай шаги вверх и вниз. Затем проверь порядок.",
    up: "Вверх",
    down: "Вниз",
    check: "Проверить",
    ...
    help: "Move steps up or down, then check the order.",
    up: "Move up",
    down: "Move down",
    check: "Check order",
  ...
  function checkOrder() {
    ...
    if (nextAttempts >= MAX_ORDERING_ATTEMPTS) {
      setOrder([...canonical]);
      setStatus("revealed");
    } else {
      setStatus("wrong");
  ...
  return <section className="g4-ordering" data-g4-ordering-state={status}>
    ...
    <ol className="g4-order-list">
      {order.map((item, index) => <li key={item}>
        <span className="g4-order-text">{item}</span>
        <span className="g4-order-controls">
          <button type="button" disabled={index === 0 || canContinue} aria-label={`${copy.up}: ${item}`} onClick={() => setOrder((current) => move(current, index, index - 1))}>↑</button>
          <button type="button" disabled={index === order.length - 1 || canContinue} aria-label={`${copy.down}: ${item}`} onClick={() => setOrder((current) => move(current, index, index + 1))}>↓</button>
        </span>
      </li>)}
    </ol>
    {status === "wrong" && <p className="g4-state-message" role="alert">{copy.wrong}</p>}
    {status === "correct" && <p className="g4-state-message" role="status">{copy.correct}</p>}
...
function FeedbackCard({ host, locale, feedback, drill }: {
  host: HTMLElement;
  locale: LocaleCode;
  ...
  const copy = locale === "ru" ? {
    correctTitle: "Верно",
    actionCorrectTitle: "Действие верное",
    actionCorrectSub: "Причину нужно уточнить",
    ...
    actionCorrectTitle: "Action is correct",
    actionCorrectSub: "The reason needs refinement",
  ...
  const title = fullyCorrect
    ? copy.correctTitle
    : feedback.actionOk
      ? copy.actionCorrectTitle
      : feedback.reasonOk
        ? copy.reasonCorrectTitle
        : copy.wrongTitle;
  ...
  return <div className="g4-feedback-card" data-g4-feedback-state={state} role="status" aria-live="polite">
    <div className="g4-feedback-status">
      <span aria-hidden="true">{fullyCorrect ? "✓" : partial ? "!" : "×"}</span>
    ...
    {showExplanation && <p className="support">{drill.explanation}</p>}
    {fullyCorrect && feedback.confidence < 50 && <p className="g4-state-message">{copy.lowConfidence}</p>}
  ...
  return <>
    <style>{`
      ...
      @media (max-width: 520px) {
        .g4-order-list { padding-left: 1.15rem; }
        .g4-order-list li { grid-template-columns: 1fr; }
        .g4-order-controls { justify-content: flex-end; }
      }
      @media (prefers-reduced-motion: reduce) {
        .g4-ordering *, .g4-feedback-card * { scroll-behavior: auto !important; transition: none !important; animation: none !important; }
      }
    `}</style>
    <OrderingPortal snapshot={snapshot} />
    <FeedbackPortal snapshot={snapshot} />
  </>;
}
