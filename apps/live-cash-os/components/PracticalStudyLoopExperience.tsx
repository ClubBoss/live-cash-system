...
export default function PracticalStudyLoopExperience() {
  ...
  return <main style={{ maxWidth: 980, margin: "0 auto", padding: "28px 20px 60px" }}>
    ...
    <details className="surface" style={{ marginTop: 22 }}>
      <summary style={{ cursor: "pointer" }}><strong>{locale === "ru" ? "Как устроен полный цикл из шести шагов" : "How the full six-step review loop works"}</strong></summary>
      <div className="section-head" style={{ marginTop: 16 }}><p className="eyebrow">{locale === "ru" ? "ЦИКЛ РАБОТЫ" : "REVIEW LOOP"}</p><h2>{locale === "ru" ? "Шесть шагов" : "Six steps"}</h2></div>
      {practicalStudyLoop.map((step, index) => <article key={step.id} className="today-card" style={{ marginTop: 12 }}><p className="eyebrow">{index + 1}</p><h3>{locale === "ru" ? step.titleRu : step.titleEn}</h3><p>{locale === "ru" ? step.instructionRu : step.instructionEn}</p><p className="support">{locale === "ru" ? `Как проверяем: ${step.learnerEvidenceRuleRu}` : `How we check: ${step.learnerEvidenceRule}`}</p></article>)}
    </details>
