...
const UNBOUND_ORIGIN_TTL_MS = 10_000;
const BOUND_ORIGIN_TTL_MS = 30 * 24 * 60 * 60 * 1_000;
...
function bindOrigin(origin: StoredSessionOrigin, session: ActiveSessionIdentity) {
  const value: StoredSessionOrigin = {
    ...origin,
...
function parseRealHandDraft(raw: string | null): StoredRealHandDraft | null {
  ...
  try {
    ...
    if (!isRecord(value)
      ...
      || value.updatedAt > Date.now() + 5 * 60 * 1_000
      || Date.now() - value.updatedAt > REAL_HAND_DRAFT_TTL_MS) return null;
    const values = value.values;
    if (!isRecord(values)) return null;

    const stringKeys: Array<Exclude<DraftField, "confidence" | "populationReadConfidence">> = [
      "moduleId",
      "stakes",
      "heroPosition",
      "villainPositions",
    ...
    const strings = Object.fromEntries(stringKeys.map((key) => [key, boundedText(values[key])])) as Record<string, string | null>;
    if (stringKeys.some((key) => strings[key] === null)) return null;

    return {
      version: 1,
      updatedAt: value.updatedAt,
      values: {
        moduleId: strings.moduleId!,
        ...
        confidence: finitePercent(values.confidence, 65),
        populationRead: strings.populationRead!,
        populationReadConfidence: finitePercent(values.populationReadConfidence, 50),
      },
    };
  } catch {
    return null;
...
function controlsFor(form: HTMLElement): ValueControl[] {
  return Array.from(form.querySelectorAll<HTMLElement>("select, input, textarea"))
    .map((control) => control as ValueControl);
}
...
function fieldControl(form: HTMLElement, key: DraftField): ValueControl | null {
  const control = form.querySelector<HTMLElement>(`[data-wave-b-draft-field="${key}"]`);
  return control ? control as ValueControl : null;
}
...
function numericControlValue(form: HTMLElement, key: "confidence" | "populationReadConfidence", fallback: number): number {
  ...
  const value = control ? Number(control.value) : Number.NaN;
...
function nativeSetValue(control: ValueControl, value: string) {
...
function restoreDraft(form: HTMLElement, draft: StoredRealHandDraft) {
  ...
  const restore = (key: DraftField, value: string | number) => {
    const control = fieldControl(form, key);
    if (!control) return;
    if (key === "moduleId" && control instanceof HTMLSelectElement
      && !Array.from(control.options).some((option) => option.value === value)) return;
...
function formLooksReset(form: HTMLElement): boolean {
  bindDraftFields(form);
  return !hasDraftContent(snapshotDraft(form).values);
}
...
function localCopy(locale: LocaleCode) {
  return locale === "ru" ? {
    ...
    saved: "Черновик сохранён только на этом устройстве. Он не создаёт прогресс или доказательство навыка.",
    ...
    saved: "Draft saved on this device only. It does not create progress or skill evidence.",
...
function RealHandDraftTools({ locale, draftPresent }: { locale: LocaleCode; draftPresent: boolean }) {
  ...
  return <div className="w7-review-inbox" data-testid="real-hand-draft-tools">
    <p className="support" data-testid="real-hand-draft-status">{draftPresent ? copy.saved : copy.localOnly}</p>
    <button
      type="button"
      className="textbutton"
      onClick={() => window.dispatchEvent(new Event("live-cash-os:clear-real-hand-draft"))}
    >
      {copy.clear}
    </button>
    <details data-testid="real-hand-example">
      <summary>{copy.exampleSummary}</summary>
      <div className="answer-panel">
        <p><b>{copy.exampleDisclaimer}</b></p>
        <p>{copy.example.stakes}</p>
        <p>{copy.example.positions}</p>
        <p>{copy.example.stack}</p>
        <p>{copy.example.straddle}</p>
        <p>{copy.example.sequence}</p>
        <p>{copy.example.board}</p>
        <p>{copy.example.sizings}</p>
        <p>{copy.example.cue}</p>
        <p>{copy.example.action}</p>
        <p>{copy.example.reason}</p>
      </div>
    </details>
  </div>;
}
  ...
  useEffect(() => {
    ...
    const reconcileOrigin = () => {
      ...
      if (origin.sessionStartedAt) {
        ...
        if (session) {
          if (session.startedAt !== origin.sessionStartedAt || (origin.mode && session.mode !== origin.mode)) {
            clearOrigin();
            return;
          }
    ...
    const handleDraftEvent = (event: Event) => {
      ...
      if (!(target instanceof HTMLElement) || !target.dataset.waveBDraftField) return;
      const form = target.closest<HTMLElement>(".field-form");
      if (form) persistDraft(form);
    };

    const scheduleSuccessfulSaveCleanup = (form: HTMLElement) => {
      let attempts = 0;
      const inspect = () => {
        attempts += 1;
        ...
        if (attempts < 12) window.setTimeout(inspect, 100);
      };
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if ((tabIndex === 1 || tabIndex === 4) && !readActiveSession()) clearOrigin();
      ...
      if (form && button.matches("button.primary") && !button.disabled) {
        ...
        scheduleSuccessfulSaveCleanup(form);
      }
    };
