...
export default function Home() {
  ...
  return <main>
    ...
    {mode === "home" && <>
      <section className="hero"><p className="eyebrow">ADAPTIVE TABLE TRAINING</p><h1>Think in trees.<br/><em>Play the next decision.</em></h1><p className="lede">A compact practice system for live cash: depth, range ancestry, action, reason, confidence, and honest evidence.</p><button className="primary" onClick={beginDiagnostic}>{state.diagnostic.status === "NOT_STARTED" ? "New here? Start T1 diagnostic" : state.diagnostic.status === "IN_PROGRESS" ? "Continue your T1 diagnostic" : "Open your T1 handoff"} <span>→</span></button></section>
      <section className="onboarding"><div><p className="eyebrow">START HERE · ONE CLEAR ROUTE</p><h2>First visit? Take T1 before studying.</h2><p>T1 is not a pass/fail exam. It captures your current decision process so the system can avoid wasting time on material you already own.</p></div><ol><li><span>01</span><div><b>Take T1 cold</b><p>Answer ten table decisions from your current process. No charts, no feedback between items.</p></div></li><li><span>02</span><div><b>Train the next mechanism</b><p>Use short five-decision blocks in the order the route unlocks.</p></div></li><li><span>03</span><div><b>Prove it travels</b><p>Use delayed recall and reviewed field notes before calling a skill stable.</p></div></li></ol></section>
