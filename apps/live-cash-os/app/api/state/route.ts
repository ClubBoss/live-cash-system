...
import { STATE_SCHEMA_VERSION, migrateLearnerState, validateLearnerState, type LearnerState } from "../../../lib/model";
...
function migrateStoredState(value: unknown): LearnerState | null {
  if (!isRecord(value) || value.schemaVersion !== STATE_SCHEMA_VERSION) return null;
...
export async function POST(request: Request) {
  ...
  const rawState = isRecord(payload) && "state" in payload ? payload.state : null;
