...
const expectedSourceRefs = new Map(decisionIds.map((id) => [id, ["FTGU-E17"]]));
...
test("PF07 UNIT_ALL_ACTIVE_FIELDS_CLASSIFIED=TRUE and UNIT_REVIEW_COUNT=0", () => {
  assert.equal(rawById.size, 8);
  assert.equal(practicalRuSystemicPreflopAdvancedPf07DecisionPatches.size, 8);

  const patches = decisionIds.map((id) => {
    const patch = practicalRuSystemicPreflopAdvancedPf07DecisionPatches.get(id);
    assert.ok(patch, `missing PF-07 patch for ${id}`);
    return [id, patch];
  });
...
test("PF07 closed PF-06 map remains frozen by exact IDs and owned paths", () => {
  assert.deepEqual([...practicalRuSystemicPreflopAdvancedPf06DecisionPatches.keys()], closedPf06Ids);
  for (const id of closedPf06Ids) {
    const patch = practicalRuSystemicPreflopAdvancedPf06DecisionPatches.get(id);
    assert.ok(patch, `missing closed PF-06 patch ${id}`);
    assert.deepEqual(patchPaths(patch), fieldPaths, `${id} closed owned paths changed`);
  }
  assert.deepEqual([...practicalRuSystemicPreflopAdvancedPf07DecisionPatches.keys()], decisionIds);
});

test("PF07 UNIT_ALL_FIX_ITEMS_REPAIRED=TRUE", () => {
  for (const id of decisionIds) {
    const finalDecision = practicalDecisionById.get(id);
    const patch = practicalRuSystemicPreflopAdvancedPf07DecisionPatches.get(id);
    assert.ok(finalDecision, `missing final-composed ${id}`);
    assert.ok(patch, `missing PF-07 patch ${id}`);
...
test("PF07 UNIT_CONFIRMED_RU_HYBRID_DEFECTS_REMAINING=0", () => {
  for (const id of decisionIds) {
...
test("PF07 LEARNER_VISIBLE_SOURCE_ID_RESIDUAL=0", () => {
...
test("PF07 NUMERIC_SEMANTICS_CHANGED=FALSE", () => {
  for (const id of decisionIds) {
...
test("PF07 semantic firewall preserves scoring, sources, option, EN, misconception and changed-variable identity", () => {
  for (const id of decisionIds) {
    const raw = rawById.get(id);
    const finalDecision = practicalDecisionById.get(id);
