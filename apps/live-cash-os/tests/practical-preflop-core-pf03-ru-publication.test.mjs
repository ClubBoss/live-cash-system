...
import { practicalRuSystemicPreflopCorePf03DecisionPatches } from "../content/practical-mastery/practical-ru-systemic-preflop-core-publication";
...
test("PF03 UNIT_ALL_ACTIVE_FIELDS_CLASSIFIED=TRUE and UNIT_REVIEW_COUNT=0", () => {
  ...
  const patches = decisionIds.map((id) => {
    const patch = practicalRuSystemicPreflopCorePf03DecisionPatches.get(id);
    assert.ok(patch, `missing PF-03 systemic preflop-core patch for ${id}`);
    return [id, patch];
  });
...
test("PF03 UNIT_ALL_FIX_ITEMS_REPAIRED=TRUE", () => {
  for (const id of decisionIds) {
    const finalDecision = practicalDecisionById.get(id);
    const patch = practicalRuSystemicPreflopCorePf03DecisionPatches.get(id);
    assert.ok(finalDecision, `missing final-composed ${id}`);
    assert.ok(patch, `missing PF-03 systemic preflop-core patch ${id}`);
...
test("PF03 UNIT_CONFIRMED_RU_HYBRID_DEFECTS_REMAINING=0", () => {
  for (const id of decisionIds) {
...
test("PF03 PF03_LEARNER_VISIBLE_SOURCE_ID_RESIDUAL=0", () => {
...
test("PF03 numeric meaning remains explicit across the RU projection", () => {
  const pf101 = practicalDecisionById.get("PM-PF-03-101");
  const pf104 = practicalDecisionById.get("PM-PF-03-104");
  const pf107 = practicalDecisionById.get("PM-PF-03-107");
  assert.ok(pf101 && pf104 && pf107);
  assert.match(fieldValue(pf101, "reason:r2"), /100%/u);
  assert.match(fieldValue(pf104, "action:b"), /50%/u);
  assert.match(pf107.cueRu, /на четверых/u);
});

test("PF03 final composition preserves scoring, source, option, EN, misconception and changed-variable identity", () => {
  for (const id of decisionIds) {
    ...
    assert.deepEqual(finalDecision.sourceRefs, ["FTGU-E04"], `${id} frozen source family`);
