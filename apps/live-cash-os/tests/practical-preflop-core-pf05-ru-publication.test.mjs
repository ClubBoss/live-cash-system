...
function numericTokens(text) {
  const withoutNonQuantitativeNotation = text
    ...
    .replace(/(?:3-bet(?:-or-fold)?|3-бет)/giu, "");
...
test("PF05 UNIT_ALL_ACTIVE_FIELDS_CLASSIFIED=TRUE and UNIT_REVIEW_COUNT=0", () => {
  ...
  const patches = decisionIds.map((id) => {
    const patch = practicalRuSystemicPreflopCorePf05DecisionPatches.get(id);
    assert.ok(patch, `missing PF-05 systemic preflop-core patch for ${id}`);
    return [id, patch];
  });
...
test("PF05 closed-unit maps remain frozen and PF-05 owns only its seven IDs", () => {
  ...
  assert.deepEqual([...practicalRuSystemicPreflopCorePf05DecisionPatches.keys()], decisionIds);
});

test("PF05 UNIT_ALL_FIX_ITEMS_REPAIRED=TRUE", () => {
  for (const id of decisionIds) {
    const finalDecision = practicalDecisionById.get(id);
    const patch = practicalRuSystemicPreflopCorePf05DecisionPatches.get(id);
    assert.ok(finalDecision, `missing final-composed ${id}`);
    assert.ok(patch, `missing PF-05 systemic preflop-core patch ${id}`);
...
test("PF05 UNIT_CONFIRMED_RU_HYBRID_DEFECTS_REMAINING=0", () => {
  for (const id of decisionIds) {
...
test("PF05 LEARNER_VISIBLE_SOURCE_ID_RESIDUAL=0", () => {
...
test("PF05 NUMERIC_SEMANTICS_CHANGED=FALSE", () => {
  for (const id of decisionIds) {
...
test("PF05 semantic firewall preserves scoring, source, option, EN, misconception and changed-variable identity", () => {
  for (const id of decisionIds) {
    const raw = rawById.get(id);
    const finalDecision = practicalDecisionById.get(id);
    ...
    assert.deepEqual(finalDecision.sourceRefs, ["FTGU-E06"], `${id} frozen source family`);
