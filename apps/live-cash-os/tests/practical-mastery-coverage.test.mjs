...
test("practical mastery has a scored decision contract and all four decision corpora", async () => {
  ...
  for (const file of ["decisions-w1-w3", "decisions-w4-w13", "decisions-w14"]) assert.match(index, new RegExp(file));
...
test("scored decisions preserve source-purity boundaries", async () => {
  ...
  assert.match(corpus, /ballpark baseline|not a law|не закон/);
  assert.match(corpus, /pool hypothesis|field-dependent|requiring validation/);
});
