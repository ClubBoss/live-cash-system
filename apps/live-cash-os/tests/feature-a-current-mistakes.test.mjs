...
function syntheticDecision({
  ...
  return {
    ...
    actionOptions: [
      ...
      { id: "b", textRu: "wrong", textEn: "wrong", misconception: actionWrongTag },
    ],
    reasonOptions: [
      { id: "r1", textRu: "correct", textEn: "correct" },
      { id: "r2", textRu: "wrong", textEn: "wrong", misconception: reasonWrongTag },
    ],
    correctActionId: "a",
    correctReasonId: "r1",
    explanationRu: "",
...
test("untagged wrong is excluded from Feature A but retained as scheduler-only SKILL fallback", () => {
  const decision = syntheticDecision({ id: "FEATURE-A-UNTAGGED", actionWrongTag: undefined, reasonWrongTag: undefined });
  withSyntheticDecisions([decision], () => {
    const state = createPracticalMasteryState();
