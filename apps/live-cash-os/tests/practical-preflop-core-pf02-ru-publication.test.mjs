...
const approvedUnpatchedFields = [
  "PM-PF-02-102:questionRu",
  "PM-PF-02-103:questionRu",
  "PM-PF-02-105:action:a",
  "PM-PF-02-105:action:b",
  "PM-PF-02-105:action:c",
  "PM-PF-02-106:action:c",
].sort();
...
test("PF02 UNIT_ALL_ACTIVE_FIELDS_CLASSIFIED=TRUE and UNIT_REVIEW_COUNT=0", () => {
  assert.equal(rawById.size, 7);

  const patches = decisionIds.map((id) => {
    const patch = practicalRuSystemicPreflopCoreDecisionPatches.get(id);
    assert.ok(patch, `missing systemic preflop-core patch for ${id}`);
    return [id, patch];
  });
  ...
  assert.equal(fixFields, 57);
  assert.equal(activeFields - fixFields, 6);
  assert.deepEqual(unpatched, approvedUnpatchedFields);
});

test("PF02 UNIT_ALL_FIX_ITEMS_REPAIRED=TRUE", () => {
  for (const id of decisionIds) {
    const finalDecision = practicalDecisionById.get(id);
    const patch = practicalRuSystemicPreflopCoreDecisionPatches.get(id);
    assert.ok(finalDecision, `missing final-composed ${id}`);
    assert.ok(patch, `missing systemic preflop-core patch ${id}`);
...
test("PF02 UNIT_CONFIRMED_RU_HYBRID_DEFECTS_REMAINING=0", () => {
  for (const id of decisionIds) {
...
test("PF02 PF02_LEARNER_VISIBLE_SOURCE_ID_RESIDUAL=0", () => {
...
test("PF02 final composition preserves scoring, source, option, EN, misconception and changed-variable identity", () => {
  for (const id of decisionIds) {
