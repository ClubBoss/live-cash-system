...
test("release gate now carries dedicated Practical Mastery browser evidence", async () => {
  ...
  for (const route of ["/mastery", "/mastery/journey", "/mastery/session", "/mastery/perception", "/mastery/study", "/mastery/reference"]) assert.match(acceptance, new RegExp(route.replaceAll("/", "\\/")));
  assert.match(acceptance, /390, height: 844/);
  assert.match(acceptance, /schemaVersion: 3/);
