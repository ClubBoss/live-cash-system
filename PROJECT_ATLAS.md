# Live Cash System - Project Atlas

Status: `ACTIVE / HUMAN_AND_AGENT_NAVIGATION_MAP`

## Fast navigation

| Need | Open first |
|---|---|
| New chat or agent | `START_HERE.md`, then `AGENTS.md` |
| Machine-readable state | `state/CURRENT_PROJECT_STATE.yaml` |
| Current source coverage | source-family `source-registry.md` |
| Missing source continuity | source-family `source-gap-ledger.md` |
| Grade 3 exam routing | `synthesis/CARROT_G3_EXAM_PRELIMINARY_COMPETENCY_MAP_v0_1.md` |
| Grade 3 Batch 05 delta | `synthesis/CARROT_G3_BATCH_05_CROSS_SOURCE_DELTA_v1.md` |
| Current Carrot evidence | `synthesis/CARROT_EVIDENCE_MATRIX_v0_5.md` |
| Candidate statuses | `synthesis/HEURISTIC_CANDIDATE_REGISTRY_v0_3.md` |
| Adaptive learning state | `learning/README.md` |
| Current readiness | `learning/ADAPTIVE_MODULE_READINESS_MANIFEST_v0_7.md` |
| Latest milestone | `reports/CARROT_G3_BATCH_05_TERMINAL_REPORT_v1.md` |

## Whole-system map

```text
external source package
-> sources/       source-faithful records and registries
-> analysis/      technical QA and contradiction review
-> synthesis/     relations -> questions -> candidates -> provisional slots
-> ranges/        independent assumptions, validation and anchors
-> learning/      modules, learner state, diagnostics and drills
-> profiles/      conditional opponent/environment overlays
-> fieldwork/     observations and falsifiers
-> hands/         real-hand records and reviewed nodes
-> playbook/      provisional and eventually admitted execution rules
```

## Source-family atlas

### Smash Live Cash

- canonical corpus complete;
- claim-driven visual checks only;
- strongest advanced evidence for depth, multiway, 3-bet pots and range composition.

### From the Ground Up

- `30/30` complete;
- strongest complete foundational sequence;
- charts are private reference only.

### Cash Injection

- `10/10` complete;
- ten population hypotheses remain field-gated.

### Carrot Poker

- Grade 1 complete;
- Grade 2 complete;
- Grade 3 Lectures `01-10` complete;
- Grade 3 Final Exam and Feedback complete;
- lecture and answer-key continuity complete;
- `74` original Carrot assessment families;
- `10` Grade 3 feedback repair paths.

Completed Grade 3 operating scaffold:

```text
ACTION HISTORY
-> CURRENT RANGE STATE
-> PURE / MIX GATE
-> VALUE / BLUFF / FRAIL CLASS
-> BET EV VERSUS CHECK EV
-> CHECK-BRANCH TRADE-OFF
-> WORLD FAVOURABILITY
-> CALL / RAISE / FOLD THRESHOLDS
-> VALUE REGION AND INVESTMENT CEILING
-> SIZE TOOLKIT
-> TOP-END VALUE / RAISE ELIGIBILITY
-> BLOCKER-FUNCTION VECTOR
-> JAM-EXPOSURE / REOPEN GATE
-> THEORY-TO-POOL CHECK
```

Exam routing:

```text
G3-Q01-Q10 -> matching primary lectures + Feedback
```

## Current strategic inventory

```text
34 heuristic candidates
-> 9 consolidation lanes
-> 16 provisional synthesis slots
-> final rule set selected without a numerical target
```

The 16 slots are non-binding containers, not a desired final count.

Statuses:

- `27` DRILL_READY;
- `7` VALIDATION_PENDING;
- `0` ADMITTED.

Direct original drill coverage:

- `30/34` candidates;
- four source-gated gaps:
  - squeeze purification;
  - deep OOP protected calls;
  - polar preflop target folds;
  - multiway delayed aggression.

## Adaptive learning atlas

| ID | Purpose | Current state |
|---|---|---|
| `LCM-01` | Node and effective depth | core ready; exact bands pending |
| `LCM-02` | Preflop range architecture | directional only; anchors and squeeze pending |
| `LCM-03` | Blind identity and realisation | protected-check and call-quality model active; exact deep boundary pending |
| `LCM-04` | Action filtering and ownership | high-confidence active |
| `LCM-05` | Bet shape and response shape | call/fold/raise geography and small-bet defence strengthened |
| `LCM-06` | Aggression and future jobs | size gate, hybrid bets, protected checks and denial-disciplined jams strengthened |
| `LCM-07` | 3-bet/4-bet ancestry | direct 3-bet and 4-bet postflop support complete; anchors and field boundaries pending |
| `LCM-08` | Multiway structure | partial; no Carrot closure |
| `LCM-09` | River audit | call quality, bet-versus-check EV and extreme sizing strengthened |
| `LCM-10` | Opponent/environment overlays | solver-to-opponent method active; field calibration pending |
| `LCM-11` | Field transfer and repair | 74 Carrot assessments and repair layer active |

## Priority and compression policy

Priority is determined by general Max-EV, not personal discomfort or source sequence.

```text
frequency x error cost x error probability
x transfer value x learnability x evidence confidence
```

Final compression uses:

`MINIMUM COMPLEXITY SUBJECT TO NO MATERIAL EV LOSS`

## Product-facing purity boundary

Product-facing outputs must use:

- original wording;
- original examples, drills and assessments;
- independently derived or licensed ranges;
- explicit assumptions;
- no copied charts, screenshots or exam spots;
- no author-specific source sequence as the product curriculum.

## Current active path

```text
cross-corpus completeness and defect audit
-> material visual-dependency review
-> targeted reruns where justified
-> Max-EV candidate reranking
-> remaining high-value gap closure
-> independent anchors
-> learner testing
-> field testing
-> optimise rule set without count target
-> admission
```

## Verdict

`ONE_REPOSITORY / ONE_ADAPTIVE_CURRICULUM`

`CARROT_GRADES_1_TO_3_COMPLETE`

`GENERAL_POSTFLOP_CORE_MATURE_AT_MECHANISM_LEVEL`

`FINAL_RULE_COUNT_EMERGENT_NOT_FIXED`

`NEXT_MILESTONE_CROSS_CORPUS_AUDIT`
